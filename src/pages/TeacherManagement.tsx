...
import { 
  ...
  Check, BookOpen, GraduationCap, Settings, 
  MessageSquare, Calendar, ClipboardCheck, Layout, TrendingUp, UserCheck
} from 'lucide-react';
import { motion, AnimatePresence } from 'motion/react';
import { Teacher, TeacherPermissions, Language, Institution, Class } from 'types';
...
const TeacherManagement: React.FC<TeacherManagementProps> = ({ lang, madrasah, onBack, role, profile }) => {
  const isTeacher = role === 'teacher';
  const canManageTeachers = !isTeacher || profile?.permissions?.can_manage_teachers;
    ...
    return (
      <div className="flex flex-col items-center justify-center h-[60vh] text-center p-6">
        ...
        <h2 className="text-xl font-black text-slate-800 mb-2">
          {lang === 'bn' ? 'আপনার অনুমতি নেই' : 'Access Denied'}
        </h2>
        ...
        <button onClick={onBack} className="px-8 py-3 bg-slate-800 text-white rounded-full font-black">
  ...
  const handleAddTeacher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.message || 'Failed to add teacher');
      }
      ...
      console.error('Create teacher error details:', err);
  ...
  const handleUpdateTeacher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.message || 'Failed to update teacher');
      }
      ...
      console.error('Update teacher error details:', err);
      setError(err.message || 'An unexpected error occurred');
    } finally {
      setSaving(false);
    }
    ...
    <div className={`min-h-screen pb-20 ${isDark ? 'bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      ...
      <div className="max-w-5xl mx-auto p-4 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {[
            { label: lang === 'bn' ? 'মোট শিক্ষক' : 'Total Teachers', value: teachers.length, icon: <Users size={20} />, color: 'blue' },
            { label: lang === 'bn' ? 'সক্রিয়' : 'Active', value: teachers.filter(t => t.is_active).length, icon: <CheckCircle2 size={20} />, color: 'emerald' },
            { label: lang === 'bn' ? 'নিষ্ক্রিয়' : 'Inactive', value: teachers.filter(t => !t.is_active).length, icon: <X size={20} />, color: 'red' },
            { label: lang === 'bn' ? 'পারমিশন প্রাপ্ত' : 'With Permissions', value: teachers.filter(t => t.permissions && Object.values(t.permissions).some(v => v === true)).length, icon: <Shield size={20} />, color: 'purple' },
          ].map((stat, idx) => (
            <motion.div 
              key={idx}
              ...
              className={`p-4 rounded-[2rem] border flex flex-col items-center text-center gap-2 ${isDark ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-100 shadow-sm'}`}
            >
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${stat.color === 'blue' ? 'bg-blue-50 text-blue-600' : stat.color === 'emerald' ? 'bg-emerald-50 text-emerald-600' : stat.color === 'red' ? 'bg-red-50 text-red-600' : 'bg-purple-50 text-purple-600'}`}>
                {stat.icon}
              </div>
              <div>
                <h4 className={`text-xl font-black ${isDark ? 'text-white' : 'text-slate-900'}`}>{stat.value}</h4>
            ...
            {filteredTeachers.map((teacher) => (
              ...
              >
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest ${teacher.is_active ? 'bg-emerald-50 text-emerald-600 border border-emerald-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
                    ...
                    {teacher.permissions && Object.entries(teacher.permissions)
                      ...
                      .map(([key]) => (
                        <div key={key} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-bold ${isDark ? 'bg-slate-800 text-slate-400' : 'bg-slate-100 text-slate-500'}`}>
                          <Shield size={10} />
                          {key.replace('can_manage_', '').replace('can_send_', '').replace('can_use_', '').replace('_', ' ')}
                        </div>
                      ))}
                    {teacher.permissions?.allowed_classes && teacher.permissions.allowed_classes.length > 0 && (
                      <div className={`flex items-center gap-1 px-2 py-1 rounded-lg text-[9px] font-bold ${isDark ? 'bg-blue-900/20 text-blue-400' : 'bg-blue-50 text-blue-600'}`}>
                        <BookOpen size={10} />
                        {teacher.permissions.allowed_classes.length} {lang === 'bn' ? 'টি শ্রেণি' : 'Classes'}
                      </div>
                    )}
                  </div>
                </div>
              ...
              <form onSubmit={showAddModal ? handleAddTeacher : handleUpdateTeacher} className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar">
                ...
                <div className="space-y-4">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {[
                      { key: 'can_manage_students', label: t('permissions_students', lang), icon: <Users size={14} /> },
                      { key: 'can_manage_attendance', label: t('permissions_attendance', lang), icon: <ClipboardCheck size={14} /> },
                      { key: 'can_manage_exams', label: t('permissions_exams', lang), icon: <GraduationCap size={14} /> },
                      { key: 'can_manage_accounting', label: t('permissions_accounting', lang), icon: <Save size={14} /> },
                      { key: 'can_send_sms', label: t('permissions_sms', lang), icon: <MessageSquare size={14} /> },
                      { key: 'can_manage_classes', label: lang === 'bn' ? 'শ্রেণি ব্যবস্থাপনা' : 'Manage Classes', icon: <Layout size={14} /> },
                      { key: 'can_use_voice_call', label: lang === 'bn' ? 'ভয়েস কল ব্যবহার' : 'Use Voice Call', icon: <Smartphone size={14} /> },
                      { key: 'can_manage_teachers', label: lang === 'bn' ? 'শিক্ষক ব্যবস্থাপনা' : 'Manage Teachers', icon: <UserCheck size={14} /> },
                      { key: 'can_manage_settings', label: lang === 'bn' ? 'সেটিংস ব্যবস্থাপনা' : 'Manage Settings', icon: <Settings size={14} /> },
                      { key: 'can_view_reports', label: lang === 'bn' ? 'রিপোর্ট দেখা' : 'View Reports', icon: <TrendingUp size={14} /> },
                    ].map((perm) => (
                      <motion.label 
                        key={perm.key} 
                        whileHover={{ scale: 1.02 }}
                        whileTap={{ scale: 0.98 }}
                        className={`flex items-center justify-between p-4 rounded-2xl border cursor-pointer transition-all ${permissions[perm.key as keyof TeacherPermissions] ? (isDark ? 'bg-blue-900/20 border-blue-900/50' : 'bg-blue-50 border-blue-200 shadow-sm') : (isDark ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-100')}`}
                      >
                        <div className="flex items-center gap-3">
                          <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${permissions[perm.key as keyof TeacherPermissions] ? (isDark ? 'bg-blue-500/20 text-blue-400' : 'bg-blue-100 text-blue-600') : (isDark ? 'bg-slate-700 text-slate-400' : 'bg-slate-200 text-slate-500')}`}>
                            {perm.icon}
                          </div>
                          <span className={`text-xs font-bold ${isDark ? 'text-slate-200' : 'text-slate-700'}`}>{perm.label}</span>
                        </div>
                        <div className={`w-10 h-6 rounded-full p-1 transition-colors duration-300 ${permissions[perm.key as keyof TeacherPermissions] ? 'bg-blue-600' : 'bg-slate-300 dark:bg-slate-700'}`}>
                          <div className={`w-4 h-4 bg-white rounded-full transition-transform duration-300 ${permissions[perm.key as keyof TeacherPermissions] ? 'translate-x-4' : 'translate-x-0'}`} />
                        </div>
                        <input 
                          type="checkbox" 
                          className="hidden"
                ...
                <div className="space-y-4">
                  <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] px-1">
                    {lang === 'bn' ? 'শ্রেণি ভিত্তিক পারমিশন' : 'Class Based Permissions'}
                  </h3>
                  <div className={`p-4 rounded-[2rem] border ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-slate-50 border-slate-100'}`}>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-4 px-1">
                      {lang === 'bn' ? 'শিক্ষক কোন কোন শ্রেণির তথ্য দেখতে পারবেন?' : 'Which classes can this teacher access?'}
                    </p>
                    <div className="grid grid-cols-2 gap-2">
                      {classes.map((cls) => {
                        const isSelected = permissions.allowed_classes?.includes(cls.id);
                        return (
                          <button
                            key={cls.id}
                            type="button"
                            onClick={() => {
                              const current = permissions.allowed_classes || [];
                              const next = isSelected 
                                ? current.filter(id => id !== cls.id)
                                : [...current, cls.id];
                              setPermissions({ ...permissions, allowed_classes: next });
                            }}
                            className={`flex items-center gap-2 px-3 py-2.5 rounded-xl border text-[11px] font-bold transition-all ${isSelected ? (isDark ? 'bg-blue-900/30 border-blue-500 text-blue-400' : 'bg-blue-600 border-blue-600 text-white shadow-lg shadow-blue-600/20') : (isDark ? 'bg-slate-900 border-slate-700 text-slate-400' : 'bg-white border-slate-200 text-slate-600')}`}
                          >
                            {isSelected ? <CheckCircle2 size={14} /> : <BookOpen size={14} />}
                    ...
                    {classes.length === 0 && (
                      <p className="text-center py-4 text-xs font-bold text-slate-400 italic">
                        {lang === 'bn' ? 'কোনো শ্রেণি পাওয়া যায়নি' : 'No classes found'}
                      </p>
                    )}
                  </div>
                </div>
                ...
                <div className="space-y-4">
                  ...
                  <div className={`flex p-1 border rounded-2xl ${isDark ? 'bg-slate-800 border-slate-700' : 'bg-slate-100 border-slate-200'}`}>
                    <button 
                      ...
                      className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${formData.is_active ? (isDark ? 'bg-slate-700 text-emerald-400 shadow-sm' : 'bg-white text-emerald-600 shadow-sm') : 'text-slate-400'}`}
                    >
                      Active
                    </button>
                    <button 
                      ...
                      className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${!formData.is_active ? (isDark ? 'bg-slate-700 text-red-400 shadow-sm' : 'bg-white text-red-600 shadow-sm') : 'text-slate-400'}`}
                    >
                      Inactive
                    </button>
                  </div>
