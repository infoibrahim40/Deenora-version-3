...
-- 1. Add pin, permissions, and user_id columns to teachers table if they don't exist
-- Also handle potential column renames if needed, but let's stick to adding for now to avoid breaking things
...
BEGIN
  ...
  -- We check both 'phone' and 'mobile' for backward compatibility
  SELECT t.id, t.institution_id, COALESCE(t.name, t.teacher_name), t.user_id
  INTO v_teacher_id, v_inst_id, v_name, v_user_id
  FROM public.teachers t
  WHERE (t.phone = p_phone OR t.mobile = p_phone) 
    AND (t.pin = p_pin OR t.password_hash = crypt(p_pin, t.password_hash)) 
    AND (t.is_active = true OR t.status = 'active');
  ...
  IF v_user_id IS NULL THEN
    ...
    INSERT INTO auth.users (
      ...
      raw_app_meta_data, raw_user_meta_data, is_confirmed
    ) VALUES (
      v_user_id, 'authenticated', 'authenticated', v_email, crypt(p_pin, gen_salt('bf')), now(), now(), now(), v_instance_id,
      '{"provider":"email","providers":["email"]}'::jsonb, '{}'::jsonb, true
    );
  ...
  SELECT 
    ...
    COALESCE(t.name, t.teacher_name) AS name,
    COALESCE(t.phone, t.mobile) AS phone,
    COALESCE(t.is_active, t.status = 'active') AS is_active,
...
-- 3. Update RLS policies for teachers table
