-- 1. Fix institution_id type in teachers table
ALTER TABLE public.teachers ALTER COLUMN institution_id TYPE UUID USING NULLIF(institution_id, '')::uuid;

-- 2. Add pin, permissions, and user_id columns to teachers table
...
-- 3. Function to check teacher login and ensure a Supabase user exists
...
-- 4. Update RLS policies for teachers table to allow teachers to read their own data
...
-- 5. Ensure profiles RLS allows teachers to read their own profile
